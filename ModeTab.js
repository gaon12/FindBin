...
export default function ModeTab() {
  ...
  useEffect(() => {
    ...
    fetch(
      "https://findbin.uiharu.dev/splash/splashs.png?timestamp=${new Date().getTime()}"
    )
      .then((response) => {
        if (response.ok) {
          setSplashImageUrl(
  ...
  useEffect(() => {
    const intervalId = setInterval(async () => {
      try {
        ...
        if (
          value !== null &&
          theme !== (value === "dark" ? DarkTheme : DefaultTheme)
        ) {
          setTheme(value === "dark" ? DarkTheme : DefaultTheme);
        }
      } catch (err) {
          ...
          screenOptions={({ route }) => ({
            ...
            tabBarStyle: {
              ...
              ...Platform.select({
                ios: {
                  borderTopWidth: 0,
                  backgroundColor: darkMode ? '#000000' : theme.colors.background,
                },
                android: {
                  elevation: 0,
                  backgroundColor:  darkMode ? '#000000' : theme.colors.background,
            ...
            tabBarActiveTintColor: darkMode ? "#fff": theme.colors.primary,
            tabBarInactiveTintColor: darkMode ? "#fff" : theme.colors.text,
            tabBarIconStyle: {
              display: "flex",
        ...
        >
          <Tab.Screen
            ...
            options={{
              ...
              headerStyle: {
                backgroundColor: darkMode ? "#000" : "#F5F5F5", // 이 부분을 수정합니다.
              },
            }}
          />
          <Tab.Screen
